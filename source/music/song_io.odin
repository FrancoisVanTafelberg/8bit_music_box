...
song_to_string :: proc(s: ^Song, allocator := context.allocator) -> string {
	...
	for &t in s.tracks {
		...
		fmt.sbprintfln(w, "instrument %s", INSTRUMENTS[t.inst].key)
...
song_parse :: proc(text: string, out: ^Song, rep: ^Load_Report, name := "song") -> bool {
	...
	for raw in strings.split_lines_iterator(&lines) {
		...
		case "track":
			append(&s.tracks, Track{name = strings.clone(args[0] if len(args) > 0 else "Track"), inst = .Violin, volume = 0.8})
			cur = &s.tracks[len(s.tracks) - 1]
			cur.pan = INSTRUMENTS[cur.inst].pan
		case "instrument", "volume", "pan", "mute", "solo", "note":
			...
			switch cmd {
			case "instrument":
				if id, found := inst_from_key(args[0] if len(args) > 0 else ""); found {
					cur.inst = id
					cur.pan = INSTRUMENTS[id].pan
				} else {
					append(&rep.warnings, fmt.aprintf("%s:%d: unknown instrument '%s', using violin", name, line_no, args[0] if len(args) > 0 else ""))
				}
			case "volume":
				if v, pok := strconv.parse_f32(args[0] if len(args) > 0 else ""); pok do cur.volume = clamp(v, 0, 2)
			case "pan":
				if v, pok := strconv.parse_f32(args[0] if len(args) > 0 else ""); pok do cur.pan = clamp(v, -1, 1)
			case "mute":
