...
to hear it; the sounding notes light while playing. Beside it: the key filter (outside the
chosen key, white keys go grey and black keys go white), **Tracking** (the last N notes up to the playhead or the selected
...
| **Single / Chords** | what the mic listens for - the button under **Tracking** on the fingerboard (in the keyboard's column too, and in the microphone settings). **Single** (the default): one note - the note you play, not its overtones: when a string's octave, twelfth or double octave rings loud enough to count as a note (a cello's open C), the lowest note they are overtones of is the one shown. **Chords**: every note it can pick out, up to four |
| **Check mode** | the **Notes** / **Check** button beside Mic. In Check, play along with the song: each note of the selected layer is judged once the playhead has passed it - outlined **green** when you played it in tune (within 20 cents) for at least half of it, **blue** when nearly (out of tune by more than 20 cents, a semitone off, in another octave, or in tune for only a third of it), **red** when you did not play it. It allows for playing a little behind the playhead (up to about 0.15 s late, 0.05 s early) - a note played late is still the right note - and a semitone-off reading that is really the note before or after (still ringing, or come early) does not count against a note. Point at a checked note to see its figures in the status line (how much of it was in tune, how much near, and by how many cents on average). Chords are checked note by note. When playback stops, the status line counts them up. The outlines stay until Play or Reset |
